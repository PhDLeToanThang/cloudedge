#!/bin/bash
###########################################################################################
#    Base Edge Cloud & Browser Access Setup Script v1.5.3                                 #
#    Written by (https://thangletoan.wordpress.com)                                       #
#    Github link: https://github.com/PhDLeToanThang/guacamole                             #
#    Contact me:  E-mail: thang.le@hotmail.com                                            #
#                                                                                         #
...
GUACAMOLE_DOWNLOAD_LINK="https://dlcdn.apache.org/guacamole/1.5.3/source/guacamole-server-1.5.3.tar.gz"
...
#Please do note that if you choose to use this script on OS other than Ubuntu 18/20/22, Debian 10, or CentOS 7/8, you might mess up your OS.  Please keep a backup of your server before installation.
...
exec > >(tee -i baseedgecloud_setup.log)
exec 2>&1
	...
	if [ -f /etc/lsb-release ] ; then
		...
		if [ $? = 0 ] ; then
      	...
      	cat /etc/lsb-release | grep "DISTRIB_RELEASE=22." >/dev/null
           ...
           say "Sorry, this script only supports Ubuntu 18,20,22, Debian 10, and CentOS 7/8." red
				echo 
				exit 1
        fi
			fi
...
#https://github.com/PhDLeToanThang/guacamole/blob/main/colored-echo.sh
	echo_content=$1
	case $2 in
		black | k ) colorf=0 ;;
		red | r ) colorf=1 ;;
...
function get_user_options
{
	...
	if [ "x$install_nginx" != "xn" ] && [ "x$install_nginx" != "xN" ] ; then
		echo 
		say @B"Please tell me your domain name (e.g., labs.company.vn):" yellow
...
function install_guacamole_ubuntu_debian
{
	...
	if [ -f $CurrentDir/guacamole-server-$GUACAMOLE_VERSION/config.status ] ; then
		...
		echo "Please also consider to report your log here https://github.com/PhDLeToanThang/guacamole/issues so that I can fix this issue."
		echo "Thank you!"
		echo 
		exit 1
	fi
...
function install_rdp
{
	...
	if [ "$OS" = "UBUNTU18" ] || [ "$OS" = "UBUNTU20" ] || [ "$OS" = "UBUNTU22" ]; then
		say @B"Please note that if you are asked to configure LightDM during this step, simply press Enter." yellow
		echo 
		echo "Press Enter to continue."
		read catch_all
...
function main
{
	...
	echo 
	echo "Thank you for using this script written by https://thangletoan.wordpress.com!"
